...
public class InputParser implements Closeable {
	...
	static {
		DEFAULT_CHANNEL_MODE_HANDLERS = ImmutableList.<ChannelModeHandler> builder()
				.add(new OpChannelModeHandler('o', UserLevel.OP) {
					@Override
					public void dispatchEvent(PircBotX bot, Channel channel, UserHostmask sourceHostmask,
							User sourceUser, UserHostmask recipientHostmask, User recipientUser, boolean adding) {
						Utils.dispatchEvent(bot, new OpEvent(bot, channel, sourceHostmask, sourceUser,
								recipientHostmask, recipientUser, adding));
					}
				}).add(new OpChannelModeHandler('v', UserLevel.VOICE) {
					@Override
					public void dispatchEvent(PircBotX bot, Channel channel, UserHostmask sourceHostmask,
							User sourceUser, UserHostmask recipientHostmask, User recipientUser, boolean adding) {
						Utils.dispatchEvent(bot, new VoiceEvent(bot, channel, sourceHostmask, sourceUser,
								recipientHostmask, recipientUser, adding));
					}
					...
					public void dispatchEvent(PircBotX bot, Channel channel, UserHostmask sourceHostmask,
							User sourceUser, UserHostmask recipientHostmask, User recipientUser, boolean adding) {
						Utils.dispatchEvent(bot, new HalfOpEvent(bot, channel, sourceHostmask, sourceUser,
								recipientHostmask, recipientUser, adding));
					}
					...
					public void dispatchEvent(PircBotX bot, Channel channel, UserHostmask sourceHostmask,
							User sourceUser, UserHostmask recipientHostmask, User recipientUser, boolean adding) {
						Utils.dispatchEvent(bot, new SuperOpEvent(bot, channel, sourceHostmask, sourceUser,
								recipientHostmask, recipientUser, adding));
					}
					...
					public void dispatchEvent(PircBotX bot, Channel channel, UserHostmask sourceHostmask,
							User sourceUser, UserHostmask recipientHostmask, User recipientUser, boolean adding) {
						Utils.dispatchEvent(bot, new OwnerEvent(bot, channel, sourceHostmask, sourceUser,
								recipientHostmask, recipientUser, adding));
					}
				}).add(new ChannelModeHandler('k') {
					@Override
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
							PeekingIterator<String> params, boolean adding, boolean dispatchEvent) {
						if (adding) {
							String key = params.next();
							channel.setChannelKey(key);
							if (dispatchEvent)
								Utils.dispatchEvent(bot,
										new SetChannelKeyEvent(bot, channel, sourceHostmask, sourceUser, key));
						} else {
							String key = params.hasNext() ? params.next() : null;
							channel.setChannelKey(null);
							if (dispatchEvent)
								Utils.dispatchEvent(bot,
										new RemoveChannelKeyEvent(bot, channel, sourceHostmask, sourceUser, key));
						}
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
						...
						if (adding) {
							...
							if (dispatchEvent)
								Utils.dispatchEvent(bot,
										new SetChannelLimitEvent(bot, channel, sourceHostmask, sourceUser, limit));
						} else {
							channel.setChannelLimit(-1);
							if (dispatchEvent)
								Utils.dispatchEvent(bot,
										new RemoveChannelLimitEvent(bot, channel, sourceHostmask, sourceUser));
						}
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
							PeekingIterator<String> params, boolean adding, boolean dispatchEvent) {
						if (dispatchEvent) {
							UserHostmask banHostmask = bot.getConfiguration().getBotFactory().createUserHostmask(bot,
									params.next());
							if (adding)
								Utils.dispatchEvent(bot,
										new SetChannelBanEvent(bot, channel, sourceHostmask, sourceUser, banHostmask));
							else
								Utils.dispatchEvent(bot, new RemoveChannelBanEvent(bot, channel, sourceHostmask,
										sourceUser, banHostmask));
						}
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
						...
						if (dispatchEvent)
							if (adding)
								Utils.dispatchEvent(bot,
										new SetTopicProtectionEvent(bot, channel, sourceHostmask, sourceUser));
							else
								Utils.dispatchEvent(bot,
										new RemoveTopicProtectionEvent(bot, channel, sourceHostmask, sourceUser));
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
						...
						if (dispatchEvent)
							if (adding)
								Utils.dispatchEvent(bot,
										new SetNoExternalMessagesEvent(bot, channel, sourceHostmask, sourceUser));
							else
								Utils.dispatchEvent(bot,
										new RemoveNoExternalMessagesEvent(bot, channel, sourceHostmask, sourceUser));
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
						...
						if (dispatchEvent)
							if (adding)
								Utils.dispatchEvent(bot,
										new SetInviteOnlyEvent(bot, channel, sourceHostmask, sourceUser));
							else
								Utils.dispatchEvent(bot,
										new RemoveInviteOnlyEvent(bot, channel, sourceHostmask, sourceUser));
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
						...
						if (dispatchEvent)
							if (adding)
								Utils.dispatchEvent(bot,
										new SetModeratedEvent(bot, channel, sourceHostmask, sourceUser));
							else
								Utils.dispatchEvent(bot,
										new RemoveModeratedEvent(bot, channel, sourceHostmask, sourceUser));
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
						...
						if (dispatchEvent)
							if (adding)
								...
								Utils.dispatchEvent(bot,
										new RemovePrivateEvent(bot, channel, sourceHostmask, sourceUser));
					}
					...
					public void handleMode(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
						...
						if (dispatchEvent)
							if (adding)
								...
								Utils.dispatchEvent(bot,
										new RemoveSecretEvent(bot, channel, sourceHostmask, sourceUser));
					}
				}).build();
	}
	...
	public void handleLine(@NonNull String rawLine) throws IOException, IrcException {
		...
		if (!sourceRaw.startsWith(":")) {
			...
			configuration.getListenerManager().onEvent(new UnknownEvent(bot, line, tags.build()));
			if (!bot.loggedIn)
	 ...
	 *
	 * @param rawLine
	 *            Raw, unprocessed line from the server
	 * @param code
	 * @param target
	 * @param parsedLine
	 *            Processed line
	 * @throws IrcException
	 *             If the server rejects the bot (nick already in use or a 4** or 5** code
	 * @throws IOException
	 *             If an error occurs during upgrading to SSL
	 */
		...
		if (command.equals("PRIVMSG") && message.startsWith("\u0001") && message.endsWith("\u0001")) {
			...
			if (request.equals("VERSION"))
				...
				configuration.getListenerManager()
						.onEvent(new ActionEvent(bot, source, sourceUser, channel, target, request.substring(7)));
			else if (request.startsWith("PING "))
				// PING request
				configuration.getListenerManager()
						.onEvent(new PingEvent(bot, source, sourceUser, channel, request.substring(5)));
				...
				if (!success)
					...
					configuration.getListenerManager().onEvent(new UnknownEvent(bot, line, tags));
			} else
			...
			sourceUser = createUserIfNull(sourceUser, source);
			configuration.getListenerManager()
					.onEvent(new MessageEvent(bot, channel, target, source, sourceUser, message, tags));
			...
			configuration.getListenerManager()
					.onEvent(new PartEvent(bot, daoSnapshot, channelSnapshot, source, sourceSnapshot, message));
			...
			if (source.getNick().equals(bot.getNick()))
				...
				bot.setNick(newNick);
			configuration.getListenerManager()
					.onEvent(new NickChangeEvent(bot, source.getNick(), newNick, source, sourceUser));
		} else if (command.equals("NOTICE")) {
			// Someone is sending a notice.
			configuration.getListenerManager()
					.onEvent(new NoticeEvent(bot, source, sourceUser, channel, target, message));
			...
			configuration.getListenerManager().onEvent(
					new KickEvent(bot, channel, source, sourceUser, recipientHostmask, recipient, parsedLine.get(2)));
			...
			configuration.getListenerManager()
					.onEvent(new TopicEvent(bot, channel, oldTopic, message, source, currentTime, true));
	...
	/**
	 * This method is called by the PircBotX when a numeric response is received from the IRC server. We use this method
	 * to allow PircBotX to process various responses from the server before then passing them on to the
	...
	public void processServerResponse(int code, String rawResponse, List<String> parsedResponseOrig) {
		...
		if (code == 433) {
			...
			} // For spec-compilant servers, if were not logged in its safe to assume we don't have a valid nick on
				// connect
			else {
			...
			configuration.getListenerManager()
					.onEvent(new NickAlreadyInUseEvent(bot, usedNick, autoNewNick, autoNickChange));
			...
			configuration.getListenerManager()
					.onEvent(new TopicEvent(bot, channel, null, channel.getTopic(), setBy, date, false));
		} else if (code == RPL_WHOREPLY) {
			// EXAMPLE: 352 PircBotX #aChannel ~someName 74.56.56.56.my.Hostmask wolfe.freenode.net someNick H :0 Full
			// Name
			...
			UserHostmask curUserHostmask = bot.getConfiguration().getBotFactory().createUserHostmask(bot, null, nick,
					parsedResponse.get(2), parsedResponse.get(3));
			...
			if (rawEndingSpaceIndex == -1) {
				// parsedResponse data is trimmed, so if the index == -1, then there was no real name given and the
				// space separating hops from real name was trimmed.
			...
			Channel channel = bot.getUserChannelDao().getChannel(parsedResponse.get(1));
			configuration.getListenerManager()
					.onEvent(new UserListEvent(bot, channel, bot.getUserChannelDao().getUsers(channel), true));
			...
			// Example: 004 PircBotX sendak.freenode.net ircd-seven-1.1.3 DOQRSZaghilopswz CFILMPQbcefgijklmnopqrstvz
			// bkloveqjfI
			...
			Channel channel = bot.getUserChannelDao().getChannel(parsedResponse.get(1));
			configuration.getListenerManager()
					.onEvent(new UserListEvent(bot, channel, bot.getUserChannelDao().getUsers(channel), false));
		}
	...
	public void processMode(UserHostmask userHostmask, User user, String target, String mode) {
		if (configuration.getChannelPrefixes().indexOf(target.charAt(0)) >= 0) {
			...
			configuration.getListenerManager()
					.onEvent(new ModeEvent(bot, channel, userHostmask, user, mode, modeParsed));
		} else {
			...
			configuration.getListenerManager()
					.onEvent(new UserModeEvent(bot, userHostmask, user, targetHostmask, targetUser, mode));
		}
	}
	...
	protected static abstract class OpChannelModeHandler extends ChannelModeHandler {
		...
		public abstract void dispatchEvent(PircBotX bot, Channel channel, UserHostmask sourceHostmask, User sourceUser,
				UserHostmask recipientHostmask, User recipientUser, boolean adding);
	}
}
