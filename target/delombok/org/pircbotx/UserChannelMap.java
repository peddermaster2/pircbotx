...
public class UserChannelMap<U extends User, C extends Channel> {
	...
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(UserChannelMap.class);
	...
	public void removeUser(U user) {
		...
		for (Channel curChannel : userToChannelMap.removeAll(user)) channelToUserMap.remove(curChannel, user);
	}
	...
	public boolean containsEntry(U user, C channel) {
		...
		if (channelToUserContains != userToChannelContains) throw new RuntimeException("Map inconsistent! User: " + user + " | Channel: " + channel + " | channelToUserMap: " + channelToUserContains + " | userToChannelMap: " + userToChannelContains);
	...
	public boolean containsUser(User user) {
		...
		if (channelToUserContains != userToChannelContains) throw new RuntimeException("Map inconsistent! User: " + user + " | channelToUserMap: " + channelToUserContains + " | userToChannelMap: " + userToChannelContains);
		return channelToUserContains;
	}
	...
	public UserChannelMapSnapshot createSnapshot(Map<U, UserSnapshot> userSnapshots, Map<C, ChannelSnapshot> channelSnapshots) {
		...
		for (Map.Entry<U, C> curEntry : userToChannelMap.entries()) userToChannelSnapshotBuilder.put(userSnapshots.get(curEntry.getKey()), channelSnapshots.get(curEntry.getValue()));
		ImmutableMultimap.Builder<ChannelSnapshot, UserSnapshot> channelToUserSnapshotBuilder = ImmutableMultimap.builder();
		for (Map.Entry<C, U> curEntry : channelToUserMap.entries()) channelToUserSnapshotBuilder.put(channelSnapshots.get(curEntry.getKey()), userSnapshots.get(curEntry.getValue()));
	...
	@java.beans.ConstructorProperties({"userToChannelMap", "channelToUserMap"})
	@java.lang.SuppressWarnings("all")
	@javax.annotation.Generated("lombok")
	protected UserChannelMap(final Multimap<U, C> userToChannelMap, final Multimap<C, U> channelToUserMap) {
