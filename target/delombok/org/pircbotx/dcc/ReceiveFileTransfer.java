...
public class ReceiveFileTransfer extends FileTransfer {
	...
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ReceiveFileTransfer.class);
			...
			try {
				...
				try {
					...
					while (true) {
						...
						int bufferSize = (remainingBytes > 0 && remainingBytes < defaultBufferSize) ? (int)remainingBytes : defaultBufferSize;
						...
						outBuffer[0] = (byte)((bytesTransfered >> 24) & 255);
						outBuffer[1] = (byte)((bytesTransfered >> 16) & 255);
						outBuffer[2] = (byte)((bytesTransfered >> 8) & 255);
						outBuffer[3] = (byte)(bytesTransfered & 255);
					...
					if (java.util.Collections.singletonList(fileOutput).get(0) != null) {
						fileOutput.close();
					}
				}
			} finally {
				if (java.util.Collections.singletonList(socketOutput).get(0) != null) {
					socketOutput.close();
				}
			}
		} finally {
			if (java.util.Collections.singletonList(socketInput).get(0) != null) {
				socketInput.close();
			}
		}
	}
