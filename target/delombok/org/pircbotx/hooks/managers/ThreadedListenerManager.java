...
public class ThreadedListenerManager extends ListenerManager {
	...
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ThreadedListenerManager.class);
	...
	public ThreadedListenerManager() {
		...
		BasicThreadFactory factory = new BasicThreadFactory.Builder().namingPattern("listenerPool" + managerNumber + "-thread%d").daemon(true).build();
	...
	public void dispatchEvent(Event event) {
		synchronized (this.listeners) {
			...
			for (Listener curListener : getListenersReal()) submitEvent(pool, curListener, event);
		}
	}
	...
	public void shutdown(PircBotX bot) {
		...
		for (ManagedFutureTask curFuture : remainingTasks) try {
	...
	public class ManagedFutureTask extends FutureTask<Void> {
		...
		public ManagedFutureTask(Listener listener, Event event, Callable<Void> callable) {
			...
			this.event = event;
			if (event.getBot() != null) synchronized (runningListeners) {
