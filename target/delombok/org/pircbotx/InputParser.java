...
public class InputParser implements Closeable {
	...
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(InputParser.class);
	...
	public void handleLine(@NonNull String rawLine) throws IOException, IrcException {
		...
		UserHostmask source;
		if (StringUtils.containsAny(target, '!', '@')) source = bot.getConfiguration().getBotFactory().createUserHostmask(bot, target); else {
	...
	public void processConnect(String rawLine, String code, String target, List<String> parsedLine) throws IrcException, IOException {
		...
		log.warn("Ignoring too fast error"); else if (configuration.isCapEnabled() && code.equals("421") && parsedLine.get(1).equals("CAP")) 
		//EXAMPLE: 421 you CAP :Unknown command
		log.warn("Ignoring unknown command error, server does not support CAP negotiation"); else if (configuration.isCapEnabled() && code.equals("451") && target.equals("CAP")) {
		...
		throw new IrcException(IrcException.Reason.CannotLogin, "Received error: " + rawLine); else if (code.equals("670")) {
	...
	public void processCommand(String target, UserHostmask source, String command, String line, List<String> parsedLine, ImmutableMap<String, String> tags) throws IOException {
		...
		if (command.equals("PRIVMSG") && message.startsWith("\001") && message.endsWith("\001")) {
			sourceUser = createUserIfNull(sourceUser, source);
			String request = message.substring(1, message.length() - 1);
			if (request.equals("VERSION")) 
			// VERSION request
			configuration.getListenerManager().dispatchEvent(new VersionEvent(bot, source, sourceUser, channel)); else if (request.startsWith("ACTION ")) 
			// ACTION request
			configuration.getListenerManager().dispatchEvent(new ActionEvent(bot, source, sourceUser, channel, target, request.substring(7))); else if (request.startsWith("PING ")) 
			// PING request
			configuration.getListenerManager().dispatchEvent(new PingEvent(bot, source, sourceUser, channel, request.substring(5))); else if (request.equals("TIME")) 
			// TIME request
			configuration.getListenerManager().dispatchEvent(new TimeEvent(bot, channel, source, sourceUser)); else if (request.equals("FINGER")) 
			// FINGER request
			configuration.getListenerManager().dispatchEvent(new FingerEvent(bot, source, sourceUser, channel)); else if (request.startsWith("DCC ")) {
			...
			configuration.getListenerManager().dispatchEvent(new PrivateMessageEvent(bot, source, sourceUser, message));
			...
			configuration.getListenerManager().dispatchEvent(new PartEvent(bot, daoSnapshot, channelSnapshot, source, sourceSnapshot, message));
			...
			configuration.getListenerManager().dispatchEvent(new KickEvent(bot, channel, source, sourceUser, recipientHostmask, recipient, parsedLine.get(2)));
	...
	public void processServerResponse(int code, String rawResponse, List<String> parsedResponseOrig) {
		...
		if (code == 433) {
			...
			configuration.getListenerManager().dispatchEvent(new ModeEvent(bot, channel, null, null, mode, modeParsed));
		} else if (code == 329) {
		...
		whoisBuilder.get(parsedResponse.get(1)).registeredAs(""); else if (code == RPL_ENDOFWHOIS) {
			//End of whois
